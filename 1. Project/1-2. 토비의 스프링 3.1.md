...
### 6-3. 다이내믹 프록시와 팩토리 빈
	...
	그 덕분에 독립적이며 어드바이스와 포인트컷으로 확장 패턴을 분리할 수도 있었다.
	새로운 비즈니스 로직을 담은 서비스 클래스가 만들어져도 transactionAdvice를 그대로 재사용 할 수 있다.

### 6-5.  스프링 AOP
	지금까지 해왔던 작업은 트랜잭션 코드를 깔끔하고 효과적이게 분리해내는 것이다.
	이렇게 분리해낸 트랜잭션 코드는 투명한 부가기능 형태로 제공되어야 한다.

	하지만 아직 해결할 과제가 남아있다.
	남은 것은 부가기능의 적용이 필요한 타깃 오브젝트마다 비슷한 내용의 빈 설정정보를 추가해주는 부분이다.

	스프링은 컨테이너로서 제공하는 기능 중에서 핵심적인 부분 외에는 확장할 수 있도록 확장 포인트를 제공해준다
	관심을 가질 만한 확장 포인트는 바로 BeanPostProcessor 인터페이스를 구현해서 만드는 빈 후처리다.
	스프링은 빈 후처리기가 빈으로 등록되어 있으면 오브젝트가 생성될 때마다 빈 후처리기에 보내서 후처리 작업을 요청한다.
	이를 잘 활용하면 스프링이 생성하는 빈의 일부를 프록시로 포장하고 프록시를 빈으로 대신 등록할 수도 있다.
	이것이 자동 프록시 생성 빈 후처리기다.
	스프링은 빈을 만들 때마다 후처리기에 빈을 보낸다 DefaultAdvisorAutoProxyCreator는 빈으로 등록된 모든 어드ㅏ이저 내의 포인트컷을 이용해 전달받은 대상이 프록시 적용 대상인지 확인한다.
	적용 대상이면 프록시 생성기에게 빈에 대한 프록시를 만들게 하고 어드바이저를 연결해준다.
	빈 후처리기는 프록시가 생성되면 전달해준 빈 오브젝트 대신 프록시 오브젝트를 돌려준다.

	지금까지 트랜잭션 어드바이스를 적용한 프록시 자동생성기를 빈 후처리기 메커니즘을 통해 적용했다
	최소한 두 가지를 확인해야한다.
	첫째는 트랜잭션이 필요한 빈에 트랜잭션 부가기능이 적용됐는가이다.
	트랜잭션이 정상적으로 커밋되는 경우에는 트랜잭션 적용 여부를 확인하기가 어렵기 때문에 예외상황에서 롤백되게 함으로써 적용 여부를 테스트 해야한다.
	둘째는 아무 빈에나 트랜잭션이 부가기능이 적용된 것은 아닌지 확인해야 한다.
	프록시 자동생성기가 어드바이저 빈에 연결해둔 포인트컷의 클래스 필터를 이용해서 정확히 원하는 빈에만 프록시를 생성했는지 확인이 필요하다
	

	이번에는 좀 더 편리한 포인트컷 작성 방법을 알아보자.
	지금까지의 포인트컷은 메서드의 이름과 클래스의 이름 패턴을 각각 클래스 필터와 메소드 매처 오브젝트로 비교해서 선정하는 방식이었다.
	스프링은 아주 간단하고 효과적인 방법으로 선정하는 알고리즘을 작성할 수 있는 방법을 제공한다.
	JSP의 EL과 비슷한 일종의 표현식 언어를 사용해서 작성할 수 있도록 하는 방법이다.

	지금까지는 이름으로 비교하기 위해 NameMatchClassMethodPointcut을 사용했는데
	포인트컷 표현식은 AspectJExpressionPointcut 클래스를 사용하면 된다.
	지금까지는 클래스 필터와 메소드 매처 두 가지를 각각 적용했는데 이제는 하나로 가능하다.

	 애플리케이션의 핵심적인 기능에서 부가 기능을 분리해서 애스펙트라는 모듈로 만들어서 설계하고 개발하는 방법을 애스펙트 지향 프로그래밍 AOP라고 한다
	 AOP는 OOP를 돕는 보조적인 기능이다.

	스프링은 프록시를 이용해 DI로 연결된 빈 사이에 적용해 AOP로 구현했다
	프록시 방식이 아닌 AOP는 AspectJ가 있다
	AspectJ는 간접적인 방식이 아닌 오브젝트를 직업 고쳐서 부가 긴응을 넣어준다.

	스프링의 프록시 방식의 AOP를 적용하려면 네 가지 빈을 등록해야 한다.
	1. 자동프록시생성기 -> DefaultAdvisorAutoProxyCreator 빈 등록
	2. 어드바이스 -> 부가 기능을 구현한 클래스 빈 등록
	3. 포인트컷 -> AspectJExpressionPointcut 빈 등록
	4. 어드바이저 DefaultPointcutAdvisor 빈 등록

	독자적인 트랜잭션 경계를 가진 코드에 대해 이미 진행 중인 트랜잭션이 영향을 미치는 것을 정의한 것이 트랜잭션 전파 속성이다
	모든 트랜잭션은 격리수준을 갖고 있어야 한다.

	세밀한 트랜잭션 속성의 제어가 필요한 경우를 위해 스프링이 제공하는 방법이 있다.
	스프링 3.0은 자바 5에서 등장한 애노테이션을 많이 사용한다.
